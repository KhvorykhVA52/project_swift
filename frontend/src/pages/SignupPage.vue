<template>
  <q-page class="row items-center justify-evenly auth-container">
    <form class="form neo-form" @submit.prevent="onSignUp">
      ...
      <div class="form__content" :class="{ 'form-loaded': !loading }">
        ...
        <p class="auth-switch">
          Уже есть аккаунт?
          <a href="#" @click.prevent="onReturn" class="auth-link">Войти</a>
        </p>
      </div>
    </form>
  </q-page>
  ...
  try {
    ...
    if (response?.success) {
      $q.notify({
        ...
        caption: 'Аккаунт будет активирован после проверки',
        color: 'positive',
        icon: 'check_circle'
      });
      router.push({ path: '/login' });
...
const showError = () => {
  $q.notify({
    ...
    caption: 'Возможно, логин уже занят',
    color: 'negative',
    icon: 'error'
  });
};
